...
async function runTests() {
  ...
  try {
    ...
    const ahmBranch = branchesRes.rows.find((b: any) => b.city.toLowerCase() === 'ahmedabad') || branchesRes.rows[0];
    const delBranch = branchesRes.rows.find((b: any) => b.city.toLowerCase() === 'delhi') || branchesRes.rows[1];
    const punBranch = branchesRes.rows.find((b: any) => b.city.toLowerCase() === 'pune') || branchesRes.rows[2];
    const mumBranch = branchesRes.rows.find((b: any) => b.city.toLowerCase() === 'mumbai') || branchesRes.rows[3];
    ...
    await query('DELETE FROM ledgers');
    await query('DELETE FROM builtys');

    // 3. Test Main Admin Authorization & Strict Login Security
    const adminRes = await query("SELECT * FROM users WHERE role = 'MAIN_ADMIN' LIMIT 1");
    ...
    await query("INSERT INTO users (name, mobile, address, role, password_hash, otp) VALUES ('Test User', '6666666666', 'Ahmedabad', 'USER', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', '123456') ON CONFLICT (mobile) DO NOTHING");
    ...
    await query('DELETE FROM users WHERE mobile = $1', [punSubAdminMobile]);
    ...
    try {
      const res = await fetch('http://localhost:3000/api/v1/admin/sub-admin', {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${jwt.sign({ id: adminUser.id, name: adminUser.name, mobile: adminUser.mobile, role: 'MAIN_ADMIN' }, JWT_SECRET, { algorithm: 'HS256' })}`
        },
        body: JSON.stringify({ name: 'Pune Sub Admin', mobile: punSubAdminMobile, address: 'Pune Office' }),
      });
      if (res.status === 400) errorCaught = true;
    } catch (e) {
      errorCaught = true;
    }
    ...
    const punSubRes = await query(
      "INSERT INTO users (name, mobile, address, role, branch_id, password_hash, otp) VALUES ($1, $2, $3, 'SUB_ADMIN', $4, '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', '123456') RETURNING *",
      ['Pune Sub Admin', punSubAdminMobile, 'Pune Hub', punBranch.id]
    );
    ...
    const delSubRes = await query("SELECT * FROM users WHERE role = 'SUB_ADMIN' AND mobile = '8888888888'");
    let delSubAdmin = delSubRes.rows[0];
    if (!delSubAdmin) {
      const insDel = await query(
        "INSERT INTO users (name, mobile, address, role, branch_id, password_hash, otp) VALUES ($1, $2, $3, 'SUB_ADMIN', $4, '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', '123456') RETURNING *",
        ['Delhi Sub Admin', '8888888888', 'Delhi Hub', delBranch.id]
      );
      delSubAdmin = insDel.rows[0];
    } else {
      await query('UPDATE users SET branch_id = $1 WHERE id = $2', [delBranch.id, delSubAdmin.id]);
    ...
    const b1 = await query(
      `INSERT INTO builtys (builty_number, serial_number, branch_id, destination_branch_id, user_id, source_city, destination_city, party_name, receiver_name, receiver_mobile, payment_status, builty_amount, status)
       VALUES ('BTY-AHM-0001', 1, $1, $2, $3, 'Ahmedabad', 'Delhi', 'Raj Traders', 'Delhi Imports', '9876543210', 'PENDING', 10000.00, 'PENDING') RETURNING *`,
      [ahmBranch.id, delBranch.id, adminUser.id]
    );
    ...
    const b2 = await query(
      `INSERT INTO builtys (builty_number, serial_number, branch_id, destination_branch_id, user_id, source_city, destination_city, party_name, receiver_name, receiver_mobile, payment_status, builty_amount, status)
       VALUES ('BTY-DEL-0002', 2, $1, $2, $3, 'Delhi', 'Pune', 'Delhi Logistics', 'Pune Enterprise', '9876543211', 'PENDING', 15000.00, 'PENDING') RETURNING *`,
      [delBranch.id, punBranch.id, adminUser.id]
    );
    assert(b2.rows.length === 1, 'Created Booking 2: Delhi -> Pune');

    // BTY-3: Pune -> Mumbai
    const b3 = await query(
      `INSERT INTO builtys (builty_number, serial_number, branch_id, destination_branch_id, user_id, source_city, destination_city, party_name, receiver_name, receiver_mobile, payment_status, builty_amount, status)
       VALUES ('BTY-PUN-0003', 3, $1, $2, $3, 'Pune', 'Mumbai', 'Pune Mills', 'Mumbai Retail', '9876543212', 'PENDING', 12000.00, 'PENDING') RETURNING *`,
      [punBranch.id, mumBranch.id, adminUser.id]
    );
    assert(b3.rows.length === 1, 'Created Booking 3: Pune -> Mumbai');

    // BTY-4: Mumbai -> Ahmedabad
    const b4 = await query(
      `INSERT INTO builtys (builty_number, serial_number, branch_id, destination_branch_id, user_id, source_city, destination_city, party_name, receiver_name, receiver_mobile, payment_status, builty_amount, status)
       VALUES ('BTY-MUM-0004', 4, $1, $2, $3, 'Mumbai', 'Ahmedabad', 'Mumbai Port', 'Ahmedabad Store', '9876543213', 'PENDING', 20000.00, 'PENDING') RETURNING *`,
      [mumBranch.id, ahmBranch.id, adminUser.id]
    );
    assert(b4.rows.length === 1, 'Created Booking 4: Mumbai -> Ahmedabad');

    // 6. Generate JWT Tokens for testing endpoints directly
    const adminToken = jwt.sign({ id: adminUser.id, name: adminUser.name, mobile: adminUser.mobile, role: 'MAIN_ADMIN' }, JWT_SECRET, { algorithm: 'HS256' });
    const delSubToken = jwt.sign({ id: delSubAdmin.id, name: delSubAdmin.name, mobile: delSubAdmin.mobile, role: 'SUB_ADMIN', branch_id: delBranch.id }, JWT_SECRET, { algorithm: 'HS256' });
    const punSubToken = jwt.sign({ id: punSubAdmin.id, name: punSubAdmin.name, mobile: punSubAdmin.mobile, role: 'SUB_ADMIN', branch_id: punBranch.id }, JWT_SECRET, { algorithm: 'HS256' });
    ...
    const delNumbers = resDelSubAll.builtys.map((b: any) => b.builty_number);
    assert(
      resDelSubAll.builtys.length === 2 && delNumbers.includes('BTY-AHM-0001') && delNumbers.includes('BTY-DEL-0002'),
      'Delhi Sub-Admin automatically restricted to Delhi-involved bookings (Ahmedabad->Delhi and Delhi->Pune)'
    );
    ...
    const punNumbers = resPunSubAll.builtys.map((b: any) => b.builty_number);
    assert(
      resPunSubAll.builtys.length === 2 && punNumbers.includes('BTY-DEL-0002') && punNumbers.includes('BTY-PUN-0003'),
      'Pune Sub-Admin automatically restricted to Pune-involved bookings (Delhi->Pune and Pune->Mumbai)'
    );
    ...
    const puneUserObj = resUsersList.users.find((u: any) => u.mobile === punSubAdminMobile);
