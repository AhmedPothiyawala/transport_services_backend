...
async function seedScenarios() {
  ...
  for (const p of partiesData) {
    await query(
      'INSERT INTO parties (name, mobile, gstin, address) VALUES ($1, $2, $3, $4)',
      [p.name, p.mobile, p.gstin, p.address]
    );
  }
  console.log('✓ Exactly 9 parties created successfully!');

  // Fetch registered customer user IDs
  const uRes = await query("SELECT id, name, mobile, branch_id FROM users WHERE role = 'USER' ORDER BY id ASC");
  console.log('Found registered customers:', uRes.rows);

  const ahmedUser = uRes.rows.find((u: any) => u.mobile === '9408379268') || uRes.rows[0]; // Pune
  const himanshuUser = uRes.rows.find((u: any) => u.mobile === '8000478617') || uRes.rows[1]; // Ahmedabad
  const dixitUser = uRes.rows.find((u: any) => u.mobile === '1234567892') || uRes.rows[2]; // Delhi
  ...
  for (const b of bookingsData) {
    ...
    await query(
      `INSERT INTO ledgers (party_name, receiver_mobile, builty_id, account_type, amount, balance, remarks)
       VALUES ($1, $2, $3, 'DEBIT', $4, $5, $6)`,
      [b.receiver_name, b.receiver_mobile, builtyId, b.builty_amount, b.pending_amount, `Freight Billed (Receiver: ${b.receiver_name}) - Builty #${builtyNumber}`]
    );
