...
export const getOutstandingSummary = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    memoryLedgers.forEach(l => {
      ...
      if (l.account_type === 'DEBIT') map[l.party_name].total_debit += l.amount;
      if (l.account_type === 'CREDIT') map[l.party_name].total_credit += l.amount;
...
export const addPaymentEntry = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const insertRes = await query(
      'INSERT INTO ledgers (party_name, builty_id, account_type, amount, balance, payment_method, remarks) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      [party_name, builty_id || null, account_type, amt, amt, payment_method || 'CASH', remarks || '']
    );
