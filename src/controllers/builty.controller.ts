...
export const createBuilty = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await client.query(
      'INSERT INTO ledgers (party_name, builty_id, account_type, amount, balance, remarks) VALUES ($1, $2, $3, $4, $5, $6)',
      [party_name, insertRes.rows[0].id, 'DEBIT', bAmount, pendAmount, `Builty Creation ${builtyNumber}`]
    );
...
export const updateWeight = async (req: AuthRequest, res: Response) => {
  ...
  try {
    await query('UPDATE builtys SET weight_kg = $1, updated_at = NOW() WHERE id = $2', [weight_kg, id]);
    return res.json({ status: true, message: 'Booking weight updated successfully' });
  } catch (err) {
    const item = memoryBuilties.find(b => b.id === Number(id));
    if (item) item.weight_kg = parseFloat(weight_kg);
    return res.json({ status: true, message: 'Booking weight updated successfully' });
  }
};
