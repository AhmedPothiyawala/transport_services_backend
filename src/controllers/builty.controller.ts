  ...
  try {
    ...
    if (user?.role === 'USER') {
      ...
      if (!userBranchId) {
        ...
        if (uRes.rows.length > 0) userBranchId = uRes.rows[0].branch_id;
      }
      if (userBranchId) {
        params.push(user.id);
...
export const updateWeight = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (item) {
      ...
      if (hasWeight) item.weight_kg = parseFloat(String(weight_kg));
      if (hasRate) item.rate_per_kg = parseFloat(String(rate_per_kg));
...
export const processSplitDelivery = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await query(
      ...
      [newDelivered, newPending, finalStatus, id]
    );

    // Insert Delivery Log with User Name & Mobile
    await query(
      'INSERT INTO delivery_logs (builty_id, delivery_person_name, parcels_delivered_in_batch, remaining_pending_after_batch, status, delivered_at) VALUES ($1, $2, $3, $4, $5, NOW())',
      [id, delPerson, count, newPending, finalStatus]
    );
 ...
 */
export const getAllDeliveryLogs = async (req: AuthRequest, res: Response) => {
...
const codeAttemptsMap = new Map<string, { attempts: number; lockUntil: number }>();

/**
 * Verification Code Authentication Gate for Status Updates
 */
export const verifyCodeForStatusUpdate = async (req: AuthRequest, res: Response) => {
 ...
 */
export const updateStatusByCode = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    await query(
      `UPDATE builtys 
       ...
       WHERE id = $4`,
      [finalStatus, newDelivered, newPending, builty.id]
    );
    ...
    await query(
      `INSERT INTO delivery_logs (builty_id, delivery_person_name, parcels_delivered_in_batch, remaining_pending_after_batch, status, delivered_at) 
       VALUES ($1, $2, $3, $4, $5, NOW())`,
      [builty.id, `${uName} [Code Verified]`, actualBatchCount, newPending, finalStatus]
    );
...
export const updateDriverStatus = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const existing = await query('SELECT * FROM builtys WHERE id = $1', [id]);
    ...
    await query(
      'INSERT INTO delivery_logs (builty_id, delivery_person_name, parcels_delivered_in_batch, remaining_pending_after_batch, status, delivered_at) VALUES ($1, $2, $3, $4, $5, NOW())',
      [id, userSuffix, status === 'DELIVERED' ? (builty?.pending_parcels || 1) : 0, status === 'DELIVERED' ? 0 : (builty?.pending_parcels || 1), status]
    );
