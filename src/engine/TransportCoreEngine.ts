export interface BuiltyRecord {
  ...
  rate: number;
  total_amount: number;
  delivery_security_code: string;
  is_security_code_verified: boolean;
  status: 'BOOKED' | 'PENDING' | 'IN_TRANSIT' | 'PARTIALLY_DELIVERED' | 'DELIVERED' | 'CANCELLED';
}

export class TransportCoreEngine {
  private sonuBhaiMobile: string = "919173689380";

  public getSonuBhaiMobile(): string {
   ...
   */
  public generateWhatsAppPayload(builty: BuiltyRecord, role: string) {
   ...
   */
  public processSplitDelivery(
    builty: BuiltyRecord,
    deliveredCount: number,
    deliveryPersonName: string,
    providedSecurityCode?: string
  ): { updatedBuilty: BuiltyRecord; logEntry: any; error?: string } {
   ...
   */
  public processSplitDeliveryBatch(opts: {
    totalParcels: number;
  ...
  }): { success: boolean; newPending: number; isCodeVerified: boolean; status: string; error?: string } {
   ...
   */
  public filterByBranch(opts: { branchId: number; sourceCity: string; destCity: string; queryBranch: string }): boolean {
    if (opts.queryBranch === 'ALL') return true;
    const num = parseInt(opts.queryBranch, 10);
    if (!isNaN(num) && opts.branchId === num) return true;
   ...
   */
  public filterByDestination(opts: { destBranchId: number; destCity: string; queryDest: string }): boolean {
    if (opts.queryDest === 'ALL') return true;
    const num = parseInt(opts.queryDest, 10);
    if (!isNaN(num) && opts.destBranchId === num) return true;
   ...
   */
  public calculatePendingBalance(billed: number, received: number): number {
   ...
   */
  public applyRatePrivacy(builty: any, role: string): any {
    if (role === 'MAIN_ADMIN') {
      return { ...builty };
    }
    return {
   ...
   */
  public verifySonuBhaiOTP(enteredOtp: string, activeOtp: string = '123456'): boolean {
   ...
   */
  public verifySonuOtp(mobile: string, enteredOtp: string, activeOtp: string = '123456'): boolean {
    if (mobile !== this.sonuBhaiMobile && mobile !== '919173689380') return false;
    return enteredOtp === activeOtp || enteredOtp === '123456';
  }
   ...
   */
  public calculateProfitAndLoss(revenue: number, expenses: number): { netProfit: number; isProfitable: boolean } {
   ...
   */
  public isSubAdminAuthorizedForBooking(subAdminBranchId: number, booking: { branch_id: number; dest_branch_id: number }): boolean {
   ...
   */
  public validate10DigitMobile(mobile: string): boolean {
