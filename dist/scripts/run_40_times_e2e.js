...
async function makeRequest(path, method = 'GET', body, token) {
...
async function runSingleIteration(iter, token) {
    // 1. Health Check (Root level /health)
    const healthRes = await new Promise((resolve, reject) => {
        http_1.default.get('http://localhost:3000/health', (res) => {
            let data = '';
            res.on('data', chunk => data += chunk);
    ...
    if (otpRes.status !== 200)
        throw new Error(`Sonu OTP verification failed: ${JSON.stringify(otpRes)}`);
    ...
    if (ledgerRes.status !== 200)
        throw new Error(`Ledger fetch failed: ${JSON.stringify(ledgerRes)}`);
    // 4. Fetch Builty Bookings (Rate Privacy Scoping)
    const builtyRes = await makeRequest('/builty/list', 'GET', undefined, token);
    if (builtyRes.status !== 200)
        throw new Error(`Builty list fetch failed: ${JSON.stringify(builtyRes)}`);
    // 5. Fetch Branch Employees & Salary Roster
    const empRes = await makeRequest('/employees', 'GET', undefined, token);
    if (empRes.status !== 200)
        throw new Error(`Employee list fetch failed: ${JSON.stringify(empRes)}`);
    ...
    if (pnlRes.status !== 200)
        throw new Error(`Profit & Loss fetch failed: ${JSON.stringify(pnlRes)}`);
    // 7. Fetch Dashboard Stats
    const statsRes = await makeRequest('/reports/dashboard-stats', 'GET', undefined, token);
    if (statsRes.status !== 200)
        throw new Error(`Dashboard stats fetch failed: ${JSON.stringify(statsRes)}`);
...
async function execute40Runs() {
    ...
    if (!token) {
        ...
        token = jwt.sign({ id: 1, name: 'Sonu Sir', mobile: '9999999999', role: 'MAIN_ADMIN', session_version: 1 }, 'transport_management_super_secret_jwt_key_2026', { algorithm: 'HS256' });
    }
