...
const createBranch = async (req, res) => {
    ...
    try {
        const dbRes = await (0, pool_1.query)('INSERT INTO branches (branch_name, city, code) VALUES ($1, $2, $3) RETURNING *', [cleanName, cleanCity, cleanCode]);
        ...
        if (err.code === '23505' && err.detail && err.detail.includes('(id)=')) {
            try {
                await (0, pool_1.query)("SELECT setval('branches_id_seq', COALESCE((SELECT MAX(id) FROM branches), 1))");
                const retryRes = await (0, pool_1.query)('INSERT INTO branches (branch_name, city, code) VALUES ($1, $2, $3) RETURNING *', [cleanName, cleanCity, cleanCode]);
...
const deleteBranch = async (req, res) => {
    ...
    try {
        ...
        const userCheck = await (0, pool_1.query)('SELECT COUNT(*) FROM users WHERE branch_id = $1', [id]);
        const builtyCheck = await (0, pool_1.query)('SELECT COUNT(*) FROM builtys WHERE branch_id = $1 OR destination_branch_id = $1', [id]);
