...
exports.getUsersList = exports.createCustomer = exports.createSubAdmin = void 0;
...
const createSubAdmin = async (req, res) => {
    ...
    try {
        ...
        const dbRes = await (0, pool_1.query)("INSERT INTO users (name, mobile, address, role, branch_id, password_hash, otp) VALUES ($1, $2, $3, 'SUB_ADMIN', $4, $5, '123456') RETURNING id, name, mobile, address, role, branch_id, is_active, created_at", [name, cleanMobile, address || '', branchIdNum, hashedPassword]);
...
const createCustomer = async (req, res) => {
    ...
    if (req.user?.role === 'SUB_ADMIN') {
        ...
        if (!subBranchId) {
            const uRes = await (0, pool_1.query)('SELECT branch_id FROM users WHERE id = $1', [req.user.id]);
    ...
    try {
        const existingCheck = await (0, pool_1.query)('SELECT id FROM users WHERE mobile = $1', [cleanMobile]);
        if (existingCheck.rows.length > 0) {
            return res.status(400).json({ status: false, message: 'A user with this mobile number already exists.' });
        }
        const dbRes = await (0, pool_1.query)("INSERT INTO users (name, mobile, address, role, branch_id, password_hash, otp, is_active) VALUES ($1, $2, $3, 'USER', $4, $5, '123456', TRUE) RETURNING id, name, mobile, address, role, branch_id, is_active, created_at", [name.trim(), cleanMobile, address ? address.trim() : '', finalBranchId, hashedPassword]);
        return res.json({
            status: true,
